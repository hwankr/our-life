...
### 수정
- 기간 상세 캘린더 날짜 셀에서 참여자 기록 점 표시를 개선
  - 최대 4명까지 개별 점으로 표시
  - 5명 이상은 `+N` 오버플로우 배지로 축약
  - 툴팁과 상세 모달은 기존처럼 전체 기록자를 유지
...
### 변경
- 새 목표 추가(OBJECTIVE)에서 공부 목표량 입력 섹션 제거
...
### 추가
- 목표 리스트에 새로고침 버튼 추가
  - `RefreshButton` 컴포넌트 생성: 클릭 시 페이지 새로고침
  - 목표 추가/삭제 후 새로고침 버튼으로 변경사항 반영 가능
---
